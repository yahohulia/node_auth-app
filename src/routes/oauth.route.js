...
oauthRouter.get('/auth/github/callback',
  passport.authenticate('github', { session: false, failureRedirect: `${process.env.CLIENT_HOST}/login` }),
  async (req, res) => {
    const normalizedUser = userService.normalize(req.user);
    const accessToken = jwtService.sign(normalizedUser);
    const refreshToken = jwtService.signRefresh(normalizedUser);
    ...
    res.redirect(`${process.env.CLIENT_HOST}/oauth?accessToken=${accessToken}&user=${encodeURIComponent(JSON.stringify(normalizedUser))}`);
  }
);
